...
void GimbalPidInit()
{
    ...
    PID_Init(&gimbal_controller.pitch_angle_pid, 180.0f, 0, .0f, 6.5f, 0, 0, 0, 0, 0, 0, 1, NONE);
    PID_Init(&gimbal_controller.pitch_speed_pid, GM6020_MAX_CURRENT, 9000, .0f, 150.0f, 280.0f, 0, 0, 0, 0.003, 0, 1, Integral_Limit | Trapezoid_Intergral | OutputFilter);
    PID_Init(&gimbal_controller.pitch_current_pid, GM6020_MAX_VOLTAGE, 18000, 0, 1.6f, 20.0f, 0, 0, 0, 0.006, 0, 1, Trapezoid_Intergral | OutputFilter | Integral_Limit);
    ...
    PID_Init(&gimbal_controller.yaw_angle_pid, 230.0, 0, 0, 10.5f, 0, 0, 0, 0, 0, 0, 1, NONE);
    PID_Init(&gimbal_controller.yaw_speed_pid, GM6020_MAX_CURRENT, 9000, 0.0, 120.0f, 50.0f, 0, 0, 0, 0.003, 0, 1, Integral_Limit | Trapezoid_Intergral | OutputFilter);
    PID_Init(&gimbal_controller.yaw_current_pid, GM6020_MAX_VOLTAGE, 12000, 0, 1.6f, 13.5f, 0, 0, 0, 0.0018, 0, 1, Trapezoid_Intergral | OutputFilter | Integral_Limit);
...
float Gimbal_Speed_Calculate(float set_point)
{
    ...
    gimbal_controller.set_yaw_current = GIMBAL_YAW_MOTOR_SIGN * PID_Calculate(&gimbal_controller.yaw_speed_pid, gimbal_controller.gyro_yaw_speed, gimbal_controller.set_yaw_speed);
    gimbal_controller.set_yaw_vol = PID_Calculate(&gimbal_controller.yaw_current_pid, gimbal_controller.yaw_info.torque_current, gimbal_controller.set_yaw_current);
    return gimbal_controller.set_yaw_vol;
}
