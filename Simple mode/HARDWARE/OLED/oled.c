...
void oled_show(void)
{
		...
		{
			OLED_ShowString(48,0,"+",12); 
			OLED_Float(0,56,now.pitch_rad,3);			
		}
		OLED_Float(1,48,Voltage,2);						//ÏÔÊ¾µçÑ¹
		//OLED_Num2(14,3,Mode);					
		OLED_Float(2,0,now.v_left,3);					//ÏÔÊ¾ÓÒ±ßµç»úµÄËÙ¶È
		OLED_Float(2,64,now.v_right,3);					//ÏÔÊ¾×ó±ßµç»úµÄËÙ¶È m/s
		OLED_ShowString(0,3,"PWM",12);
		OLED_Num4(5,3,oled1);
		OLED_Float(3,64,oled_E,3);
		OLED_ShowString(0,4,"theta",12);
		OLED_Float(4,64,oled_A,2);
		OLED_ShowString(0,5,"D_theta: ",12);
		OLED_Float(5,64,oled_B,3);		
		OLED_ShowString(0,6,"alpha : ",12);
		OLED_Float(6,64,oled_C,3);
		OLED_ShowString(0,7,"the-alp: ",12);
		OLED_Float(7,64,oled_D,3);
		switch(Mode)
		{
			case 97:
